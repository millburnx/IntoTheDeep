...
data class Scale(val ppi: Double, val scale: Double) {
    ...
    fun ppiScale(vec2d: Vec2d): Vec2dInt = Vec2dInt((vec2d * ppi).roundToInt())
    fun ppiScale(vec2d: Vec2dInt): Vec2dInt = Vec2dInt((vec2d * ppi).roundToInt())

    fun ppiScale(pair: Pair<Number, Number>): Pair<Int, Int> = Pair(ppiScale(pair.first), ppiScale(pair.second))

    fun ppiScale(value: Number): Int = (value.toDouble() * ppi).roundToInt()
    fun ppiScale(value: Double): Int = (value * ppi).roundToInt()
    fun ppiScale(value: Float): Int = (value * ppi).roundToInt()
    fun ppiScale(value: Int): Int = (value * ppi).roundToInt()
}