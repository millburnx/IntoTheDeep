...
object ClipSettings {
    ...
    @JvmField
    var kernalSize: Double = 20.0

    @JvmField
    var minContourSize: Double = 60.0

    @JvmField
    var minContourPercent: Double = 0.5
}
...
class ClipPipeline : VisionProcessor, CameraStreamSource {
    ...
    override fun processFrame(
        ...
        // threshold
        val threshold = Mat()
        Imgproc.threshold(grayscale, threshold, ClipSettings.threshold, 255.0, Imgproc.THRESH_BINARY_INV)

        val kernel =
            Imgproc.getStructuringElement(Imgproc.MORPH_RECT, Size(ClipSettings.kernalSize, ClipSettings.kernalSize))
        ...
        val masked = Mat()
        Core.bitwise_and(frame, frame, masked, threshold)
        ...
        contours.forEach { contour ->
            ...
            if (minRect.size.width < ClipSettings.minContourSize || minRect.size.height < ClipSettings.minContourSize) {
                // too small
                contour.release()
                return@forEach
            }
            ...
            Imgproc.drawContours(masked, listOf(contour), -1, Scalar(0.0, 255.0, 0.0), 2)
            contour.release()

            if (contourArea / boundingArea < ClipSettings.minContourPercent) {
            ...
            Imgproc.polylines(masked, pointMats.toList(), true, Scalar(0.0, 255.0, 255.0), 2)
